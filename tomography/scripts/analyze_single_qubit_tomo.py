...
def barplot3d(rho, title, dpi=100, save_path=None):
    ...
    tick_labels = ['H' ,'V']
    ticks = np.arange(1.25, 3.25, 1)  # Adjusted so the ticks and labels match
...
def quad1QubitS(new_measurements_array):
    '''
    Translates data from 1-qubit scheme measurements (structured as tuples)
    ...
    # Basis labels for 1 qubit
    lb = ['Z', 'X', 'Y']

    ord = list()

    # Get projectors for each basis and flatten label tuples
    for k in lb:
        L = pLab(k)  # e.g., ['H', 'V']
        for i, p in enumerate(L):
            L[i] = ''.join(p)

    # Reorder dict to follow projector basis order
    for key in ord:
        tomodic[key] = tomodic.pop(key)

    print("Tomodic:", tomodic)
    return tomodic
...
def plot_density_matrix_from_tomo_data(data_directory, target_density_matrix, transmitted_col, reflected_col, arm_name):
    ...
    tomodic = quad1QubitS(new_measurements)
...
if __name__ == "__main__":
    ...
    # Ensure the target density matrix is a 2x2 matrix
    if target_density_matrix.shape != (2, 2):
        print("Error: Target density matrix must be a 2x2 matrix.")
        print("Example: [[0.5, 0.5], [0.5, 0.5]]")
        sys.exit(1)
