...
class fitHOM:
    def __init__(self, position, coincidences):
        ...
        bkgd_guess = np.mean(np.sort(yData)[int(round(0.90*len(yData)))::]) # average 90 percentile largest values
        xpos_guess = xData[np.argmin(yData)] # use min(coincidences) index to guess dip position
        ...
        slope_guess = (yData[0] - yData[-1]) / (xData[0]-xData[-1]) # slope from rise over run, first and final data point
        yint_guess = yData[0] - slope_guess * xData[0] # y-intercept extrapolated from guessed slope and first data point
        ...
        res_g = np.dot((yData - self.func_gaus(xData, *popt_g)),(yData - self.func_gaus(xData, *popt_g)))
        res_g1 = np.dot((yData - self.func_gaus1(xData, *popt_g1)),(yData - self.func_gaus1(xData, *popt_g1)))
        res_t = np.dot((yData - self.func_tri(xData, *popt_t)),(yData - self.func_tri(xData, *popt_t)))
        res_t1 = np.dot((yData - self.func_tri1(xData, *popt_t1)),(yData - self.func_tri1(xData, *popt_t1)))
        ...
        print('R^2 for Gaussian: %.4f and Triangle: %.4f and Gaussian+Linear: %.4f and Triangle+Linear: %.4f' %(float(r_sqr_g), float(r_sqr_t), float(r_sqr_g1), float(r_sqr_t1)))

        # generate curvefit datapoints
        nPoints = 500
        cfit_g = self.func_gaus(np.linspace(min(xData), max(xData), nPoints), *popt_g)
        cfit_g1 = self.func_gaus1(np.linspace(min(xData), max(xData), nPoints), *popt_g1)
        ...
        plt.legend((dat_g, cfit_g),('data','fit: ampl=%.0f, xpos=%.4f, sigm=%.4f, bkgd=%.0f' %tuple(popt_g)))
    ...
    def func_tri1(x, ampl, xpos, sigL, sigR, slope, yint):
        ...
        for xx in x:
            if xx < (xpos-sigL):
                ...
                ret.append((slope*xx+yint)-((xx-(xpos-sigL))/(xpos-(xpos-sigL))*ampl))
            elif xpos <= xx < (xpos+sigR):
                ret.append((slope*xx+yint)-(((xpos+sigR)-xx)/((xpos+sigR)-xpos))*ampl)
            elif xx >= (xpos+sigR):                
                ret.append(slope*xx+yint)
        return np.array(ret)
...
if __name__ == '__main__':
    ...
    try:
        ...
        while True:
            try:
                xString = input('Specify scan position column name %s: ' % str(tuple(df.columns)))
                position = df[xString]
                break
            except SyntaxError:
    ...
    if not valid_columns:  # If none of the columns are found, prompt the user to specify
        while True:
            try:
                yString = input('Specify coincidences column name(s), separated by commas: %s: ' % str(tuple(df.columns)))
                valid_columns = [col.strip() for col in yString.split(',') if col.strip() in df.columns]
                if valid_columns:
                    break
                else:
