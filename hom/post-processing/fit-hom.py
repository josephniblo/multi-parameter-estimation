...
class FitHOM:
    def __init__(self, position, coincidences):
        ...
        bkgd_guess = np.mean(
            np.sort(y_data)[int(round(0.90 * len(y_data))) : :]
        ...
        slope_guess = (y_data[0] - y_data[-1]) / (
            x_data[0] - x_data[-1]
        )  # slope from rise over run, first and final data point
        yint_guess = (
            y_data[0] - slope_guess * x_data[0]
        ...
        popt_g1, pcov_g1 = curve_fit(
            self.func_gaussian_linear, x_data, y_data, p0=param_guess_g1
        )
        ...
        popt_t1, pcov_t1 = curve_fit(
            self.func_triangle_linear, x_data, y_data, p0=param_guess_t1
        )
        ...
        res_g = np.dot(
            (y_data - self.func_gaussian(x_data, *popt_g)),
            (y_data - self.func_gaussian(x_data, *popt_g)),
        )
        res_g1 = np.dot(
            (y_data - self.func_gaussian_linear(x_data, *popt_g1)),
            (y_data - self.func_gaussian_linear(x_data, *popt_g1)),
        )
        res_t = np.dot(
            (y_data - self.func_triangle(x_data, *popt_t)),
            (y_data - self.func_triangle(x_data, *popt_t)),
        )
        res_t1 = np.dot(
            (y_data - self.func_triangle_linear(x_data, *popt_t1)),
            (y_data - self.func_triangle_linear(x_data, *popt_t1)),
        )
        ...
        n_points = 500
        cfit_g = self.func_gaussian(
            np.linspace(min(x_data), max(x_data), n_points), *popt_g
        )
        cfit_g1 = self.func_gaussian_linear(
            np.linspace(min(x_data), max(x_data), n_points), *popt_g1
        )
        cfit_t = self.func_triangle(
            np.linspace(min(x_data), max(x_data), n_points), *popt_t
        )
        cfit_t1 = self.func_triangle_linear(
            np.linspace(min(x_data), max(x_data), n_points), *popt_t1
        )
    ...
    def func_triangle_linear(x, ampl, xpos, sig_l, sig_r, slope, yint):
        ...
        for xx in x:
            if xx < (xpos - sig_l):
                ...
                ret.append(
                    (slope * xx + yint)
                    - ((xx - (xpos - sig_l)) / (xpos - (xpos - sig_l)) * ampl)
                )
            elif xpos <= xx < (xpos + sig_r):
                ret.append(
                    (slope * xx + yint)
                    - (((xpos + sig_r) - xx) / ((xpos + sig_r) - xpos)) * ampl
                )
            elif xx >= (xpos + sig_r):
                ret.append(slope * xx + yint)
        return np.array(ret)
