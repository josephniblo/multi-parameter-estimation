...
"""
Created on Sat Feb 25 14:34:23 2023
...
def fit_sin(tt, yy):
    ...
    r_sqr_s = 1 - res_s / tot
    return {"amp": A, "omega": w, "phase": p, "offset": c, "freq": f, "period": 1./f, "fitfunc": fitfunc, "maxcov": numpy.max(pcov), "rawres": (guess,popt,pcov), "res_s": res_s,
            "r_sqr_s": r_sqr_s}
...
def plot_cfit2(xData, yData, cfitData, nPoints,monte):
    ...
    # draw confidence region around curvefit defined by one-sigma assuming Poissonian statistics
    plt.fill_between(np.linspace(min(x),max(x), n), cfit - monte, cfit + monte, alpha=0.3, color='y')
...
stdErr_vis_s = ((perr_s[0] / res["amp"])**2 + (perr_s[3] / res["rawres"][1][-1])**2 )**0.5 * visi

plt.errorbar(dfFOUR['angle'],df['0dd_coincidences'], yerr=MONTE_Err,ls='None',fmt='None')
...
plt.title('Visi (\%): ' + str(round(visi*100,4)) + '+-' + str(round(stdErr_vis_s*100,4)))
plt.show()
...
def fit_sin(tt, yy):
    ...
    popt, pcov = scipy.optimize.curve_fit(sin_func, tt, yy, 
...
def find_minimum(fit_params, x_range):
    ...
    result = scipy.optimize.minimize_scalar(neg_sin, bounds=(min(x_range), max(x_range)), method='bounded')
...
plt.axvline(dip_position, color='blue', linestyle='--', label=f"Dip at {dip_position:.2f}°")
plt.text(dip_position, np.min(yy), f"  {dip_position:.2f}°", color='blue', verticalalignment='bottom')
